...
def init_db():
    ...
    try:
        ...
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_ip_ranges_scan_cidr ON ip_ranges (scan_id, cidr);"
        )  # Index composite key
        ...
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_subdomains_fqdn ON subdomains (fqdn);"
        )
...
def save_scan_result(result: ReconnaissanceResult) -> bool:
    ...
    try:
        ...
        try:
            ...
            if not scan_id:
                raise sqlite3.Error(
                    "Failed to get last row ID after inserting scan record."
                )
            logger.info(f"Inserted scan record with ID: {scan_id}")
        except sqlite3.Error as e:
            logger.exception(
                f"Database error during insert_scan_record for '{result.target_organization}': {e}"
            )
            if conn:
                conn.rollback()
            return False
        ...
        try:
            if result.asns:
                ...
                logger.debug(
                    f"Inserted/ignored {inserted_ignored_count} ASNs into 'asns' table."
                )
                ...
                logger.info(
                    f"Successfully linked {linked_count} ASNs to scan {scan_id}."
                )
                if linked_count != len(result.asns):
                    logger.warning(
                        f"Mismatch in ASN linking: Expected {len(result.asns)}, linked {linked_count}. This might indicate an issue if ASNs were expected to be linked but weren't."
                    )
        except sqlite3.Error as e:
            logger.exception(
                f"Database error during save_asn_data for '{result.target_organization}': {e}"
            )
            if conn:
                conn.rollback()
            return False
            ...
            if result.ip_ranges:
                ...
                for ipr in result.ip_ranges:
                    key = (
                        scan_id,
                        ipr.cidr,
                    )  # Key for deduplication based on UNIQUE constraint
                ...
                if not ip_range_tuples:
                    logger.warning(
                        "No unique IP ranges remained after deduplication step."
                    )
                else:
                    logger.debug(
                        f"Attempting to insert {len(ip_range_tuples)} unique IP ranges after filtering."
                    )
                    cursor.executemany(
                        """
                    ...
                    logger.info(
                        f"Database operation affected {affected_count} IP Range rows for scan {scan_id} (tried: {len(ip_range_tuples)} unique ranges)."
                    )
                    # No warning needed here as duplicates were pre-filtered
        except sqlite3.Error as e:
            logger.exception(
                f"Database error during save_ip_range_data for '{result.target_organization}': {e}"
            )
            if conn:
                conn.rollback()
            return False
        ...
        try:
            if result.domains:
                total_subdomains_to_save = sum(
                    len(d.subdomains) for d in result.domains
                )
                logger.info(
                    f"Saving {len(result.domains)} Domains and {total_subdomains_to_save} associated Subdomains..."
                )
                saved_domains = 0
                saved_subdomains = 0
                for domain in result.domains:
                    ...
                    logger.debug(
                        f"Inserted domain '{domain.name}' with ID {domain_id} for scan {scan_id}."
                    )

                    # Insert Subdomains for this Domain
                    if domain.subdomains:
                        logger.debug(
                            f"Preparing to insert {len(domain.subdomains)} subdomains for domain ID {domain_id}..."
                        )
                        subdomain_data = [
                            (
                                domain_id,
                                ...
                                sub.status,
                                (
                                    json.dumps(sorted(list(sub.resolved_ips)))
                                    if sub.resolved_ips
                                    else None
                                ),
                                sub.data_source,
                        ...
                        if subdomain_data:
                            logger.debug(
                                f"Sample subdomain data for domain {domain_id}: {subdomain_data[0]}"
                            )
                        ...
                        if not deduplicated_subdomain_data:
                            logger.debug(
                                f"No unique subdomains remained for domain '{domain.name}' after deduplication."
                            )
                        else:
                            if len(deduplicated_subdomain_data) < len(subdomain_data):
                                logger.warning(
                                    f"Filtered out {len(subdomain_data) - len(deduplicated_subdomain_data)} duplicate FQDNs for domain '{domain.name}' before DB insert."
                                )

                            logger.debug(
                                f"Inserting {len(deduplicated_subdomain_data)} unique subdomains for domain ID {domain_id}..."
                            )
                            cursor.executemany(
                                """
                            ...
                            logger.debug(
                                f"Database operation affected {sub_affected} Subdomain rows for domain '{domain.name}' (tried: {len(deduplicated_subdomain_data)} unique subdomains)."
                            )
                            # No warning needed here, duplicates handled before query

                    else:
                        logger.debug(
                            f"No subdomains found for domain '{domain.name}' (ID: {domain_id}) in the result object."
                        )
                    saved_domains += 1
                    # Increment saved_subdomains by the actual number affected
                    saved_subdomains += sub_affected if domain.subdomains else 0
                logger.info(
                    f"Successfully saved {saved_domains} Domains and {saved_subdomains} Subdomains."
                )
        except sqlite3.Error as e:
            # This catches errors before the loop starts or if re-raised from inside
            logger.exception(
                f"Database error during save_domain_data for '{result.target_organization}': {e}"
            )
            if conn:
                conn.rollback()
            return False
        ...
        try:
            if result.cloud_services:
                ...
                affected_count = cursor.rowcount
                logger.info(
                    f"Successfully saved/ignored {affected_count} Cloud Services for scan {scan_id}."
                )
                if affected_count != len(cloud_service_data):
                    logger.warning(
                        f"Attempted {len(cloud_service_data)} cloud services, affected {affected_count}. UNIQUE constraint ignores likely."
                    )
        except sqlite3.Error as e:
            logger.exception(
                f"Database error during save_cloud_service_data for '{result.target_organization}': {e}"
            )
            if conn:
                conn.rollback()
            return False
        ...
        try:
            if result.warnings:
                ...
                logger.debug(
                    f"Inserted {len(warning_data)} warnings for scan {scan_id}."
                )
        except sqlite3.Error as e:
        ...
        success = True
        logger.info(
            f"Database transaction committed successfully for operation: {operation}"
        )

    except sqlite3.Error as e:
        logger.exception(
            f"Database error during operation: {operation}. Rolling back transaction."
        )
        if conn:
            conn.rollback()
        ...
        logger.exception(
            f"Unexpected error during operation: {operation}. Rolling back transaction if possible."
        )
        if conn:
            try:
                conn.rollback()
...
def get_scan_history(limit: int = 10) -> List[sqlite3.Row]:
    """Retrieves recent scan history metadata from the database with asset counts."""
    scans = []
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute(
            ...
            SELECT 
                s.scan_id, 
                s.target_organization, 
                s.scan_timestamp,
                COALESCE(domain_count.total_domains, 0) as total_domains,
                COALESCE(asn_count.total_asns, 0) as total_asns,
                COALESCE(ip_count.total_ip_ranges, 0) as total_ip_ranges,
                COALESCE(cloud_count.total_cloud_services, 0) as total_cloud_services
            FROM scans s
            LEFT JOIN (
                SELECT scan_id, COUNT(*) as total_domains 
                FROM domains 
                GROUP BY scan_id
            ) domain_count ON s.scan_id = domain_count.scan_id
            LEFT JOIN (
                SELECT scan_id, COUNT(*) as total_asns 
                FROM scan_asns 
                GROUP BY scan_id
            ) asn_count ON s.scan_id = asn_count.scan_id
            LEFT JOIN (
                SELECT scan_id, COUNT(*) as total_ip_ranges 
                FROM ip_ranges 
                GROUP BY scan_id
            ) ip_count ON s.scan_id = ip_count.scan_id
            LEFT JOIN (
                SELECT scan_id, COUNT(*) as total_cloud_services 
                FROM cloud_services 
                GROUP BY scan_id
            ) cloud_count ON s.scan_id = cloud_count.scan_id
        ...
        logger.debug(
            f"Retrieved {len(scans)} records from scan history with asset counts."
        )
    except sqlite3.Error as e:
...
def get_result_by_scan_id(scan_id: int) -> Optional[ReconnaissanceResult]:
    ...
    try:
        ...
        cursor.execute(
            "SELECT target_organization FROM scans WHERE scan_id = ?", (scan_id,)
        )
        ...
        result = ReconnaissanceResult(
            target_organization=scan_row["target_organization"]
        )
        ...
        if domains_map:  # Only query subdomains if there are domains
            ...
            logger.debug(
                f"Fetched {len(subdomain_rows)} total subdomain rows for {len(domain_ids)} domains in scan {scan_id}."
            )
            ...
            for sub_row in subdomain_rows:
                ...
                if domain_id in domains_map:
                    try:
                        resolved_ips_set = (
                            set(json.loads(sub_row["resolved_ips"]))
                            if sub_row["resolved_ips"]
                            else set()
                        )
                        ...
                        logger.error(
                            f"Failed to decode resolved_ips JSON for subdomain {fqdn} (domain_id {domain_id}): {json_err}"
                        )
                        result.add_warning(
                            f"Subdomain Load Error: Failed to parse IPs for {fqdn}"
                        )
                    except Exception as assoc_err:
                        logger.error(
                            f"Error associating subdomain {fqdn} with domain ID {domain_id}: {assoc_err}"
                        )
                        result.add_warning(
                            f"Subdomain Load Error: Failed to process {fqdn}"
                        )

                else:
                    # This shouldn't happen due to the WHERE clause, but good to log if it does
                    logger.warning(
                        f"Found subdomain '{fqdn}' for unknown domain_id {domain_id} during load for scan {scan_id}."
                    )
            logger.debug(
                f"Successfully associated {associated_count} subdomains with their parent domains."
            )

        result.domains = set(
            domains_map.values()
        )  # Get the Domain objects from the map
        ...
        # 6. Get Warnings
        cursor.execute(
            "SELECT message FROM scan_warnings WHERE scan_id = ?", (scan_id,)
        )
        ...
        logger.exception(
            f"Database error loading scan result for scan_id {scan_id}: {e}"
        )
        return None
    except Exception as e:
        logger.exception(
            f"Unexpected error loading scan result for scan_id {scan_id}: {e}"
        )
        return None
    finally:
        if conn:
...
def check_existing_scan(
    target_organization: str, max_age_hours: int = 24
) -> Optional[int]:
    ...
    try:
        ...
        if result:
            ...
            logger.info(
                f"Found recent scan (ID: {scan_id}) for target '{target_organization}'."
            )
        else:
            logger.debug(
                f"No recent scan found for target '{target_organization}' within {max_age_hours} hours."
            )

    except sqlite3.Error as e:
        logger.exception(
            f"Database error checking for existing scan for '{target_organization}': {e}"
        )
    finally:
        if conn:
            conn.close()
...
class DatabaseManager:
    """
    Database Manager class that provides an object-oriented interface
    to the reconnaissance database operations.

    This class wraps the existing function-based database operations
    to provide compatibility with the Streamlit application architecture.
    """

    def __init__(self):
        """Initialize the database manager and ensure database is set up."""
    ...
    def store_scan_result(
        self, result: ReconnaissanceResult, metadata: dict = None
    ) -> int:
        """
        Store a reconnaissance result in the database.

        Args:
            result: The ReconnaissanceResult object to store
            metadata: Optional metadata dictionary (for compatibility)

        Returns:
            int: The scan ID of the stored result
        """
        success = save_scan_result(result)
        if success:
            # Get the most recent scan ID for this organization
            recent_scans = get_scan_history(limit=1)
            if (
                recent_scans
                and recent_scans[0]["target_organization"] == result.target_organization
            ):
                scan_id = recent_scans[0]["scan_id"]
                self.logger.info(f"Stored scan result with ID: {scan_id}")
                return scan_id

        raise Exception("Failed to store scan result in database")

    def load_scan_result(self, scan_id: int) -> Optional[ReconnaissanceResult]:
        """
        Load a reconnaissance result from the database.

        Args:
            scan_id: The ID of the scan to load

        Returns:
            ReconnaissanceResult or None if not found
        """
        return get_result_by_scan_id(scan_id)
    ...
    def get_recent_scans(self, limit: int = 10) -> List[dict]:
        ...
        """
        scan_rows = get_scan_history(limit)

        # Convert sqlite3.Row objects to dictionaries with additional metadata
        scans = []
        for row in scan_rows:
            scan_dict = dict(row)

            # Add computed fields for compatibility with UI
            scan_dict["id"] = scan_dict["scan_id"]

            # ✅ FIX: Convert datetime to formatted string to prevent slicing errors
            if isinstance(scan_dict["scan_timestamp"], datetime):
                scan_dict["created_at"] = scan_dict["scan_timestamp"].strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
            else:
                scan_dict["created_at"] = str(scan_dict["scan_timestamp"])

            # Asset counts are now included from the SQL query, no need to override
    ...
    def check_existing_scan(
        self, target_organization: str, max_age_hours: int = 24
    ) -> Optional[int]:
        """
        Check if a recent scan exists for the target organization.

        Args:
            target_organization: Organization name to check
            max_age_hours: Maximum age of scan in hours

        Returns:
            Scan ID if found, None otherwise
        """
        return check_existing_scan(target_organization, max_age_hours)
    ...
    def get_scan_statistics(self) -> dict:
        ...
        try:
            ...
            cursor.execute("SELECT COUNT(*) as total_asns FROM asns")
            total_asns = cursor.fetchone()["total_asns"]

            cursor.execute("SELECT COUNT(*) as total_domains FROM domains")
            total_domains = cursor.fetchone()["total_domains"]

            cursor.execute("SELECT COUNT(*) as total_subdomains FROM subdomains")
            total_subdomains = cursor.fetchone()["total_subdomains"]
    ...
    def cleanup_old_scans(self, max_age_days: int = 30) -> int:
        """
        Clean up old scans from the database.

        Args:
            max_age_days: Maximum age of scans to keep
        ...
        try:
            ...
            self.logger.error(f"Error cleaning up old scans: {e}")
            return 0
