...
try:
    ...
    logging.getLogger(__name__).warning(
        "dnspython library not found. DNS resolution for status check will be limited."
    )
...
def _parse_crtsh_json(
    json_content: str, query: str, result: ReconnaissanceResult
) -> Set[str]:
    ...
    logger.debug(
        f"Found {len(cleaned_names)} potential domains/subdomains from crt.sh for query '{query}'"
    )
...
@with_api_backoff
def _check_and_query_hackertarget(
    domain: str, result: ReconnaissanceResult
) -> Set[str]:
    ...
    if _hackertarget_limit_tracker.get(scan_instance_id, False):
        logger.debug(
            f"Skipping HackerTarget query for {domain}: API limit previously hit in this scan."
        )
        return set()
    ...
    with rate_limiter.acquire("dnsdumpster", f"hackertarget_{domain}"):
        ...
        try:
            ...
            if response.text:
                ...
                if lines and "API count exceeded" in lines[0]:
                    ...
                    if not _hackertarget_limit_tracker.get(scan_instance_id, False):
                        logger.warning(warning_msg)
                        result.add_warning(
                            f"PassiveDNS (HackerTarget): API limit exceeded."
                        )  # Add generic warning once
                        _hackertarget_limit_tracker[scan_instance_id] = (
                            True  # Set limit hit flag for this scan
                        )
                ...
                logger.debug(
                    f"HackerTarget Passive DNS returned empty response for {domain}"
                )
            ...
            warning_msg = (
                f"Failed query to HackerTarget Passive DNS for '{domain}': {e}"
            )
            logger.warning(warning_msg)
            result.add_warning(
                f"PassiveDNS (HackerTarget): Query failed for {domain} - {e}"
            )
        except Exception as e:
            warning_msg = f"Unexpected error processing HackerTarget Passive DNS for '{domain}': {e}"
            logger.exception(warning_msg)
            result.add_warning(
                f"PassiveDNS (HackerTarget): Error processing {domain} - {e}"
            )
...
def _resolve_domain(
    fqdn: str, result: ReconnaissanceResult
) -> Tuple[Optional[str], Set[str], datetime]:
    ...
    # Early validation to avoid unnecessary DNS queries
    if not fqdn or len(fqdn) < 3:
        logger.debug(f"Skipping invalid FQDN: {fqdn}")
        return "Invalid", set(), checked_time

    # Basic domain format validation
    if not re.match(r"^[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}$", fqdn):
        logger.debug(f"Skipping malformed FQDN: {fqdn}")
        return "Invalid", set(), checked_time
    ...
    if not DNSPYTHON_AVAILABLE:
        try:
            ...
            logger.debug(f"Socket resolution failed for {fqdn}: {e}")
            return "Unknown", set(), checked_time

    resolver = dns.resolver.Resolver()
    resolver.timeout = 3.0  # Increased from 2.0 to 3.0 for better reliability
    ...
    try:
        ...
        logger.debug(f"DNS resolution timed out for {fqdn}")
        # Don't add to warnings for timeouts - these are common
        status = "Timeout"
    except dns.resolver.NoNameservers:
        logger.debug(f"DNS resolution failed for {fqdn}: No nameservers available")
        # Only log as warning if it's a well-formed domain
        if "." in fqdn and len(fqdn.split(".")) >= 2:
            result.add_warning(f"DNS: No nameservers for {fqdn}")
        status = "Inactive"
    except Exception as e:
        logger.debug(f"Unexpected DNS resolution error for {fqdn}: {e}")
        # Only add warnings for unexpected errors on valid-looking domains
        if "." in fqdn and len(fqdn.split(".")) >= 2:
            result.add_warning(f"DNS: Unexpected error for {fqdn} - {e}")
        status = "Unknown"
...
def find_domains(
    ...
    progress_logger = create_progress_logger(__name__)
    progress_logger.info("Starting domain search...")

    # Update progress callback if provided
    def update_progress(percent: float, message: str):
        progress_logger.info(f"Progress {percent:.1f}%: {message}")
    ...
    valid_domain_regex = re.compile(r"^[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}$")
    base_domains = {
        domain.lower() for domain in base_domains if valid_domain_regex.match(domain)
    }
    ...
    if crtsh_queries:
        ...
        with ThreadPoolExecutor(
            max_workers=crtsh_workers, thread_name_prefix="CrtShQuery"
        ) as executor:
            future_to_query = {
                executor.submit(_query_crtsh, query, result): query
                for query in crtsh_queries
            }
            for future in as_completed(future_to_query):
                ...
                try:
                    ...
                    if fqdns_from_crtsh:
                        ...
                        logger.info(
                            f"Got {len(fqdns_from_crtsh)} results from crt.sh for query '{query}'"
                        )
                    # else: logger.debug(f"No results from crt.sh for query '{query}'")
                except Exception as exc:
                    warning_msg = (
                        f"crt.sh query '{query}' generated an exception: {exc}"
                    )
                ...
                progress_percent = (
                    10 + (crtsh_query_count / total_crtsh_queries * 15)
                    if total_crtsh_queries > 0
                    else 25
                )
                update_progress(
                    progress_percent,
                    f"Queried CT logs {crtsh_query_count}/{total_crtsh_queries}",
                )
    else:
    ...
    logger.info(
        f"Found {len(all_domains)} potential domains/subdomains via certificate transparency logs"
    )
    ...
    if initial_count != len(all_domains):
        logger.debug(
            f"Filtered {initial_count - len(all_domains)} invalid FQDN patterns found in CT logs."
        )
    ...
    update_progress(
        30,
        f"Performing passive DNS queries for {len(passive_dns_queries)} base domains...",
    )
    ...
    for idx, query_domain in enumerate(passive_dns_queries):
        ...
        progress_percent = (
            30 + (idx / len(passive_dns_queries) * 15) if passive_dns_queries else 45
        )
        update_progress(
            progress_percent,
            f"Passive DNS query {idx+1}/{len(passive_dns_queries)}: found {len(passive_dns_results)} subdomains",
        )
    ...
    for i, (base_name, subdomains) in enumerate(grouped_domains.items()):
        ...
        update_progress(
            domain_progress,
            f"Processing domain {i+1}/{total_domains}: {base_name} with {len(subdomains)} subdomains",
        )
    ...
    for base_name, subdomains in grouped_domains.items():
        ...
        for subdomain_name in subdomains:
            all_resolution_tasks.append(
                (subdomain_name, base_domain_objects[base_name])
            )

    # Resolve DNS for all domains and subdomains
    update_progress(
        60, f"Resolving DNS for {len(all_resolution_tasks)} domains and subdomains..."
    )
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(future_to_domain):
            ...
            try:
                ...
                update_progress(
                    resolution_progress,
                    f"Resolved {completed}/{total_to_resolve}: {domain_name}",
                )
    ...
    update_progress(
        100,
        f"Completed with {len(result.domains)} domains and {total_subdomains} subdomains",
    )

    logger.info(
        f"✅ Domain discovery completed. Added {len(result.domains)} domains with {total_subdomains} subdomains."
    )
    return result
...
if __name__ == "__main__":
    ...
    # org = "Google LLC"
    org = None
    test_result = ReconnaissanceResult(target_organization="test")
    find_domains(org_name=org, base_domains=domains_input, result=test_result)

    print(f"\n--- Found Domains ({len(test_result.domains)}) ---")
    for dom in sorted(list(test_result.domains), key=lambda d: d.name):
